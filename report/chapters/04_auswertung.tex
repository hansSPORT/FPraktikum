...
Insgesamt liegen 10 auswertbare Punkte im Temperaturbereich $5{,}916~\mathrm{K} \leq T \leq 6{,}901~\mathrm{K}$ vor; ein zusätzlicher Punkt bei $T = 5{,}821~\mathrm{K}$ liefert nur eine untere Schranke $B_c > 505~\mathrm{mT}$.
...
\begin{figure}[htbp]
    ...
    Kreise: auswertbare Messpunkte (einschließlich des Punktes bei $T = 6{,}901~\mathrm{K}$, diskutiert in Abschnitt~\ref{sec:diskussion}).
    Aufwärts-Dreieck: untere Schranke bei $T = 5{,}82~\mathrm{K}$ ($B_c > 505~\mathrm{mT}$).
    Rote Kurve: parabolischer Fit~\eqref{eq:bc_parabola} mit $B_c(0) = 2779~\mathrm{mT}$, $T_c = 6{,}861~\mathrm{K}$.
    Gestrichelte graue Kurve: Pb-Bulk-Literaturwerte ($B_c(0) = 80~\mathrm{mT}$, $T_c = 7{,}2~\mathrm{K}$).}
    \label{fig:bc_phase}
\end{figure}
...
Ein Neodym-Permanentmagnet wurde über eine gekühlte YBCO-Tablette gehalten und in mehreren Zentimetern Abstand zur Oberfläche frei schwebend beobachtet.
Das Experiment wurde sowohl im Zero-Field-Cooling- als auch im Field-Cooling-Modus durchgeführt.
Im Zero-Field-Cooling-Modus levitierte der Magnet stabil; im Field-Cooling-Modus war die Levitation geringer, jedoch zeigte die Tablette deutliches Flux-Pinning: Der Magnet konnte innerhalb eines gewissen Bereichs lateral bewegt werden, ohne dass er herunterfiel.
